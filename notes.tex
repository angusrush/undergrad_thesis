...
\begin{definition}[projective module]
  ...
  \begin{equation*}
    \begin{tikzcd}
      ...
      \arrow[d, "\psi"]
      \\
      A \arrow[r, swap, twoheadrightarrow, "\varphi"]
      & R
    \end{tikzcd}
  \end{equation*}
\end{definition}
...
\begin{theorem}
  Filtered categories have all finite coproducts.
\end{theorem}
\begin{proof}
...
\end{example}
\begin{note}
  This is stated without proof as Example 3.39 of \cite{nlab-deligne-theorem}.
\end{note}
...
\begin{lemma}
  ...
  Let $\mathsf{C}$ be a category with monoidal structure $(\otimes, 1, \alpha, \lambda, \rho)$. Then the maps $\lambda_{1}$ and $\rho_{1}\colon 1 \otimes 1 \to 1$
\end{lemma}
\begin{proof}
...
\begin{definition}[finite tensor category]
  ...
  \begin{enumerate}
    \item There are only finitely many simple objects in $\mathsf{A}$, and each of them admits a projective presentation.\footnote{The definition of this is a bit tricky, and I don't understand it. However, it's satisfied trivially by pretty much every `obviously finite' category we'll look at.}
...
\begin{itemize}
  ...
  \item For $A$ any supercommutative superalgebra (i.e. a commutative monoid internal to $\mathsf{SVect}_{k}$), a \defn{supercommutative $A$-algebra} is 
\end{itemize}
...
\begin{note}
  ...
  As always, $(\mathsf{C}, \otimes, 1)$ denotes at worst a plain old monoidal category and at best a tensor category. The case we really care about is when $\mathsf{C} = \mathsf{SVect}_{k}$.

  First, we look at monoids internal to $\mathsf{C}$; these form a category called $\mathsf{Mon}(\mathsf{C})$, which has a full subcategory of commutative monoids $\mathsf{SMon}(\mathsf{C})$. When $\mathsf{C} = \mathsf{SVect}_{k}$, $\mathsf{CMon}(\mathsf{SVect}_{k})$ is nothing else but the category whose objects are supercommutative, associative $k$-superalgebras with unity, denoted $\mathsf{SCAlg}_{k}$.

  The idea is to view the category of super-spaces as dual to $\mathsf{SCAlg}_{k}$. Thus, we call 
  \begin{equation*}
...
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item $\mathsf{CMon}(\mathsf{C})^{A/}$ is the coslice category (\hyperref[def:coslicecategory]{Definition \ref*{def:coslicecategory}}) whose objects are morphisms $A \to E$, with $E$ a commutative monoid in $\mathsf{C}$.
  \end{itemize}

  Then there is an equivalence of categories
  \begin{equation*}
    \mathsf{CMon}(A\mhyp\mathsf{Mod}(\mathsf{C})) \simeq \mathsf{CMon}^{A/}(\mathsf{C}).
  \end{equation*}
\end{theorem}
\begin{proof}
...
\begin{theorem}
  Let $\mathsf{C}$ be a symmetric monoidal category such that 
\end{theorem}

The upshot of the past few theorems is the following.
\begin{theorem}
  Let $\mathsf{A}$ be a tensor category. Let $R \in \Obj(\mathsf{CMon}(\mathsf{C}))$ be a commutative monoid in $\mathsf{A}$.

  Let $A_{1}$ and $A_{2}$ be two $R$-algebras in the sense of \hyperref[def:internalalgebra]{Definition \ref*{def:internalalgebra}}. By \hyperref[def:affinesuperscheme]{Definition \ref*{def:affinesuperscheme}}, we can regard $A_{1}$ and $A_{2}$ as affine schemes, i.e. as objects $\Spec(A_{1})$, $\Spec(A_{2}) \in \mathsf{Aff}(R\mhyp\mathsf{Mod}(\mathsf{A}))$.

  Then the category $\mathsf{Aff}(R\mhyp\mathsf{Mod}(\mathsf{A}))$ has products. Further, these are dual the product $A_{1} \otimes_{R} A_{2}$, in the sense that there exists a natural isomorphism
  \begin{equation*}
    \Spec(A_{1}) \times \Spec(A_{2}) \simeq \Spec(A_{1} \otimes_{R} A_{2}).
  \end{equation*}
\end{theorem} 
...
\begin{note}
  ...
  In this chapter, we saw that vector bundles over a manifold $M$ were dual to finitely-generated, projective $C^{\infty}(M)$ modules, which led us to the definition of a quasi-coherent sheaf over an affine super scheme $\Spec(A)$ via the correspondence
  \begin{equation*}
...
Central to Deligne's theorem is the idea of an affine algebraic super-group. Na\"{i}vely, this is simply an affine super scheme which is also a group. As usual, the way to formalize this is with internal objects.
...
\begin{definition}[group object]
  ...
  \begin{itemize}
    ...
    \item The inverse map behaves as an inverse, i.e. the following diagram commutes.
      ...
      Here, $\Delta\colon G \to G \times G$ is the diagonal map defined by the universal property of the product.
  \end{itemize}
\end{definition} 

Let us first deal with an example. The functor $C^{\infty}\colon \mathsf{SmoothMfd}^{\mathrm{op}} \rightsquigarrow \mathsf{Alg}_{\R}$ defined in \hyperref[def:functorcinfinity]{Definition \ref*{def:functorcinfinity}} was fully faithful, but not essentially surjective. Thus, we can decrese the codomain of the functor $C^{\infty}$ to include only smooth algebras:
\begin{equation*}
...
\end{equation*}
Moreover, the images of the coherence diagrams for the group structure on $G$ under $C^{\infty}$ translate into dual coherence diagrams enforcing `co-associativity,' etc. This gives $C^{\infty}(G)$ the structure of a \emph{Hopf algebra}.



\chapter{Representations}
We have already seen that categories can be used to re-phrase certain aspects of representation theory. (Recall examples \ref{eg:groupsaregroupoidswithoneobject}, \ref{eg:functorscanbegrouprepresentation}, and \ref{eg:intertwinersarenaturaltransformations}.) In this section we will take a more systematic approach.

\begin{definition}
  \label{def:categoryofrepresentations}
  Let $G$ be a group, $\mathsf{G}$ the group which mimics it\footnote{For those playing along at home, this is the \emph{delooping} of $G$, usually notated by $\mathbb{B}G$.} (as in \hyperref[eg:groupsaregroupoidswithoneobject]{Example \ref*{eg:groupsaregroupoidswithoneobject}}). Recall that functors $\rho\colon \mathsf{G} \rightsquigarrow \mathsf{Vect}_{k}$ are $k$-linear representations of $G$ (\hyperref[eg:functorscanbegrouprepresentation]{Example \ref*{eg:functorscanbegrouprepresentation}}).

  Denote by $\mathsf{Rep}(G)$ the functor category $\Hom_{\mathsf{Cat}}(\mathsf{G}, \mathsf{Vect}_{k})$.
\end{definition}

\section{Linear super-representations as Comodules}
\section{Super fiber functors and their automorphism groups}
We would like to view tensor categories $\mathcal{A}$ as categories of representations. Thus, we would like to be able to view the objects of $\mathcal{A}$ as being the objects of some other category $\mathcal{V}$, plus some extra structure. Thus, there should be a forgetful functor
\begin{equation*}
  \omega\colon \mathcal{A} \rightarrow \mathcal{V}.
\end{equation*}
...
Let us consider the case in which we generalize the symmetry group of our theory to a general (algebraic) super group.

The basic idea of the previous section is unchanged. If our quantum theory has a symmetry given by some algebraic super group $G$, its Hilbert space $\mathscr{H}$ must admit a ray representation of $G$, and particles correspond to irreducible representations of $G$.

And again we can make the argument that the Hilbert space of a fundamental particle must admit an irreducible representation of $G$.

By the way, this is not so outlandish: one faces this already in the standard model! Quarks and neutrinos are not irreps of the Poincar\'{e} group. Instead they are irreps of the total symmetry group, see the previous section.

And, no matter what, these particles will transform linearly with this group.
...
For any interesting or physically realistic theory (i.e. one with interactions), scattering amplitudes are fiendishly difficult to compute; in general one must resort to expressing them as an asymptotic series, the terms of which are pictured diagrammatically. 
...
Perhaps now is a good time to make the following remark. If we wish to view two quantum systems with Hilbert spaces $\mathscr{H}_{1}$ and $\mathscr{H}_{2}$ as a single composite system, the Hilbert space we consider is $\mathscr{H}_{1} \otimes \mathscr{H}_{2}$. That is to say, composite systems are the tensor product of their constituents. And the composite system's representation of $\bar{G}$ is given by the tensor product of the representations of its constituents.
...
\begin{enumerate}
  ...
  \item If we swap the order in which we adjoin two quantum systems twice, we get the same thing we started with:
    ...
    \end{equation*}
    This means $\bar{G}$-symmetric quantum systems form a \textbf{symmetric monoidal category}.

  \item Each fundamental particle has an antiparticle, and we can construct from any composite system an anti-system, in which all of its particles are replaced by antiparticles.
    This corresponds mathematically to the fact that our Hilbert spaces $\mathscr{H}$ have duals $\mathscr{H}^{*}$ which admit the contragredient representation of $\bar{G}$.

    This means that we are dealing with a \textbf{rigid symmetric monoidal category}.
\end{enumerate}
Aficianados of category theory will notice that we have not yet said anything about what is arguably the most important aspect of a category: the morphisms.\footnote{Morphisms are a category-theoretic generalization of the idea of a function. Just as between any two sets $A$ and $B$ one has a collection of functions $A \to B$, between any two objects in a category one has a collection of morphisms $A \to B$.}
...
A note for the mathematically-inclined: the category we are working with is $\mathsf{Rep}(\bar{G})$, i.e. the functor category $\mathsf{Func}(\mathbf{B}\bar{G}, \mathsf{Vect}_{\C})$ (where $\mathbf{B}\bar{G}$ is the delooping groupoid of $\bar{G}$), i.e. the objects are functors $\mathbf{B}\bar{G} \to \mathsf{Vect}_{\C}$ and the morphisms are natural transformations. When 
...
\begin{enumerate}
  ...
  \item Looking to the future? What sort of project (if any) could I do with you next year? If not, do you know anywhere I \emph{could} do this sort of thing?

  \item Irreducible means irreducible w.r.t. direct sums. We're talking about irreducible w.r.t. tensor products. What gives?

    Elaboration: It is fine that we can factor every tensor product into direct sums, but then fundamental particles lose their interpretation as subsystems. This also seems off.

  \item For a general symmetry group, what happens to Wigner classification? Specifically, can we always (at least for a connected group) get a unitary representation by passing to some double cover?

  \item Everybody seems very cavalier about the difference between $G$ and $\bar{G}$. Why is this? Is it because they have the same Lie algebra?

  \item More like a concern: there are rather a lot of finite-dimensionality assumptions that are glossed over in \cite{physicsforums-why-deligne}. Some things aren't adding up. More specifically:
    \begin{itemize}
      \item The article says that particle species, i.e. irreducible representations, form tensor categories. But if one tensors together irreps, one gets a composite system. So in order to be closed under the tensor product, the tensor category in question must be the category of \emph{all} representations, not just irreps. Okay, so Urs was being a little bit sloppy, but that's okay; so far so good.

      \item But the symmetric braiding has to hold for everything in the tensor category, not just irreps. So we lose the interpretation of the braiding as representing particle interchange, since we have to use the same braiding no matter WHAT we're commuting. It still makes sense to demand that exchanging twice is the identity, so this isn't a fundamental problem; we still get a symmetric monoidal category.

        Actually, there is a silver lining: the fact that the braiding doesn't turn symmetric in (2 + 1)-d doesn't matter anymore.

      \item But none of this matters because the category of representations of a general (super)group isn't even a tensor category because $\mathsf{Hilb}$ isn't an abelian category. The category of finite-dimensional representations \emph{is} a tensor category, but that's not much help for the Poincar\'{e} group. So the prototypical example of what we're interested in isn't actually an example.

        Maybe this is okay; after all, a LOT of stuff goes wrong when we use non-compact groups. It'll make it a lot harder to sell, though.

      \item I guess it's possible that there's something I'm not getting. Would it be appropriate to e-mail Urs or John to ask for clarification?
    \end{itemize}

  \item Probably the thing to do is to explicitly restrict our attention to finite-dimensional representations. This makes the scope of the project a lot less impressive, but at least it'll be rigorous.
\end{enumerate}
