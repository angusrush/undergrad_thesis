...
\begin{note}
  ...
  The triangle and pentagon diagram are the first in a long list of hulking coherence diagrams designed to pacify categorical structures so that they behave like their algebraic counterparts. For a monoid, multiplication is associative by fiat, and the extension of this to $n$-fold products follows by a trivial application of induction. In monoidal categories, associators are isomorphisms rather than equalities, so we have to ensure $n$-ary associativity by hand.
...
One thinks of the functor $\mathcal{D}$ embedding the index category $M\textsf{J}$ into $\mathsf{C}$.
...
\section{Less basic definitions}
...
\begin{definition}[conormal epimorphism]
  ...
  An epimorphism $f\colon A \to B$ is \defn{conormal} if it is the image of some morphism. That is to say, if there exists an object $C$ and a morphism $g\colon C \to A$ such that $(B, f)$ is the kernel of $g$.
\end{definition}
...
\begin{example}
  ...
  As we saw in \hyperref[eg:setisamonoidalcategory]{Example \ref*{eg:setisamonoidalcategory}}, the category $\mathsf{Set}$ is a monoidal category whose bifunctor is given by the cartesian product. The internal hom is simply the regular hom functor 
  \begin{equation*}
    \Hom_{\mathsf{Set}}(-,-) = [-,-].
  \end{equation*}
...
\begin{proof}
  ...
  To check that this is functorial, we must check that the following diagram commutes.
  \begin{equation*}
    \begin{tikzcd}[row sep=huge, column sep=huge]
      Z^{*} \otimes X
...
\begin{definition}[pre-abelian category]
  ...
  A category  $\mathsf{C}$ is \defn{pre-abelian} if it is additive and every morphism has a kernel and a cokernel.
\end{definition}
...
\end{definition}
\begin{example}
  As we have seen, the category $\mathsf{Vect}_{k}$ is abelian. It is clearly additive; it is pre-abelian since the zero vector space $0$ is the zero object and there is only one linear map $0 \to 0$, the zero morphism; that it is binormal (hence abelian) follows from basic linear algebra.
\end{example}
