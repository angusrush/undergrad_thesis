...
\begin{definition}[evaluation map]
  ...
  Let $(\mathsf{C}, \otimes)$ be a closed monoidal category (\hyperref[def:closedmonoidalcategory]{Definition \ref*{def:closedmonoidalcategory}}) with internal hom functor $[-,-]_{\mathsf{C}}$. Let $X \in \Obj(\mathsf{C})$. Then the adjunction
  \begin{equation*}
    (-) \otimes X \dashv [X, -]_{\mathsf{C}}
  \end{equation*}
...
\begin{definition}[composition morphism]
  ...
  Let $(\mathsf{C}, \otimes)$ be a closed monoidal category. For $X$, $Y$, $Z \in \Obj(\mathsf{C})$, the \defn{composition morphism}
  \begin{equation*}
    \circ_{X, Y, Z}\colon [Y, Z] \otimes [X, Y] \to [X, Y]
  \end{equation*}
  is the $(-) \otimes X \vdash [X, -]$-adjunct of the composition
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      {[Y, Z]} \otimes {[X, Y]} \otimes X 
      \arrow[r, "{\left(1_{[Y, Z]}, \ev_{X, Y}\right)}"]
      & {[Y, Z]} \otimes Y
      \arrow[r, "{\ev_{Y, Z}}"]
      & Z
    \end{tikzcd}.
...
\begin{example}
  ...
  The evaluation morphism $\circ_{X, Y, Z}$ is then the currying of this, i.e. it sends
  \begin{equation*}
    (f, g) \mapsto (f \circ g)(-).
  \end{equation*}
\end{example}
...
\begin{definition}[dual object]
  ...
  Let $(\mathsf{C}, \otimes, 1)$ be a closed monoidal category, $X \in \Obj(\mathsf{C})$. The \defn{dual object} to $X$, denoted $X^{*}$, is defined to be the object
  \begin{equation*}
    [X, 1]_{\mathsf{C}}.
  \end{equation*}
  (Recall that $[-,-]_{\mathsf{C}}$ denotes the hom functor internal to $\mathsf{C}$.)

  That is to say, $X^{*}$ is the internal hom object modelling the hom set of morphisms from $X$ to the identity object $1$.
\end{definition}

The evaluation morphism has 

\begin{notation}
  In many sources, e.g. DMOS (\cite{DMOS}), the dual object to $X$ is denoted $X^{\vee}$.
\end{notation}
