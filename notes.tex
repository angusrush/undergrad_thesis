...
\begin{definition}[function module]
  ...
  For $(N_{1}, \rho{1})$ and $(N_{2}, \rho_{2})$ two $A$-modules internal to a closed symmetric monoidal category, the \defn{function module} $\hom_{A}(N_{1}, N_{1})$ is the equalizer 
\end{definition}
...
Every quantum theory which makes any attempt to describe the world we see must take into account the existence of spacetime, so every quantum system will admit the symmetries of spacetime. Thus, any quantum system can be viewed as a composite system made up a (large) number of fundamental particles which are irreducible representations of the symmetries of spacetime. Thus, quantum systems have certain properties. 
...
\begin{enumerate}
  ...
  \item The order in which we form composites does not matter since the tensor product is naturally associative:
    \begin{equation*}
      (\mathscr{H}_{1} \otimes \mathscr{H}_{2}) \otimes \mathscr{H}_{3} \simeq \mathscr{H}_{1} \otimes (\mathscr{H}_{2} \otimes \mathscr{H}_{3}).
    \end{equation*}
...
These properties, phrased in mathematical language, say that the collection of all quantum systems which admit a $G$-symmetry has the structure of a \emph{monoidal category}.
...
\begin{enumerate}
  ...
  \item More like a concern: there are rather a lot of finite-dimensionality assumptions that are glossed over in \cite{physicsforums-why-deligne}. 
\end{enumerate}
