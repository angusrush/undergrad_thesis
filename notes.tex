...
\end{theorem}
\begin{proof}
  Since finite products and equalizers are both instances of finite limits, 1 clearly implies 2. Now assume 2, i.e. suppose that $\mathsf{C}$ has all finite products and equalizers. 
...
\begin{example}
  ...
  The simplest (though not the prototypical) example of a monoidal category is $\mathsf{Set}$ with the Cartesian product. We have already studied this structure in some detail in REF. We check the axioms in \hyperref[def:monoidalcategory]{Definition \ref*{def:monoidalcategory}}.
...
\end{lemma}
\begin{proof}
  We will show that the kernel of $f - g$ satisfies the universal property for the equalizer of $f$ and $g$.
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item The morphism $f$ is mono iff $\ker(f) = 0$
    \item The morphism $f$ is epi iff $\coker(f) = 0$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  $\,$
  \begin{enumerate}
    \item We first show that if $\ker(f) = 0$, then $f$ is mono. Suppose $\ker(f) = 0$. By the universal property of kernels, we know that for any $Z \in \Obj(\mathsf{C})$ and any $g\colon Z \to A$ with $f \circ g = 0$ there exists a unique map $\bar{g}\colon Z \to \ker(f)$ such that $g = \iota \circ \bar{g}$.
      ...
      Next, we show that if $f$ is mono, then $\ker(f) = 0$. To do this, it suffices to show that $\ker(f)$ is final.

      Suppose we have two morphisms $h_{1}$ and $h_{2}\colon Z \to \ker(f)$. 
      \begin{equation*}
        \begin{tikzcd}
          Z
...
\begin{definition}[subobject, quotient object, subquotient object]
  ...
  \begin{enumerate}
    \item A \defn{subobject} of $Y$ is an object $i\colon X \in \Obj(\mathsf{C})$ together with a monomorphism $X \to Y$.
...
\begin{lemma}
  A subquotient $Z'$ of a subquotient $Z$ of an object $O$ is a subquotient of $O$.
\end{lemma}
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}
      ...
      \\
      X
      \arrow[ru, hookrightarrow]
      \arrow[rd, twoheadrightarrow]
      \\
      & Z
    \end{tikzcd}
  \end{equation*}
