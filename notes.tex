...
\begin{example}
  ...
  A monoid internal to $\mathsf{Vect}_{k}$ is is nothing else but a unitary, associative $k$-algebra. Let us see that this agrees with \hyperref[def:algebraoverafield]{Definition \ref*{def:algebraoverafield}}.
...
\begin{definition}
  Let $M$ be a $C^{\infty}$ manifold (that is, recalling our definition, an object in the category $\mathsf{SmoothMfd}$.) The functor 
  \begin{equation*}
    C^{\infty}\colon \mathsf{SmoothMfd} \rightsquigarrow \mathsf{Alg}_{\R}
  \end{equation*}
  is the defined by sending $M$ to the $\R$-algebra of smooth functions $M \to \R$, and morphisms $\varphi \in \Hom_{\mathsf{Man}^{\infty}}(M, N)$ to homomorphisms of $\R$-algebras $C^{\infty}(N) \to C^{\infty}(M)$ via the pullback. 
\end{definition}
