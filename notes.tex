...
\begin{example}
  One might suspect that the above generalizes to all sorts of categories. For example, one might hope that in $\mathsf{Set}$, the exponential $H^{G}$ of two groups $G$ and $H$ would somehow capture all homomorphisms $G \to H$. This turns out to be impossible; we will see why in REF.
\end{example}
...
\begin{example}
  ...
  \end{equation*}
  be the components of the above natural transformation, and let $g\colon X' \to X$. Naturality follows from the fact that the diagram below commutes.
  \begin{equation*}
    \begin{tikzcd}[row sep=huge, column sep=huge]
      \Hom(X, A \times B)
  ...
  \begin{equation*}
    A \amalg B \to X,
  \end{equation*}
  and vice versa. Similar reasoning yields a natural bijection between the following functors $\mathsf{C} \to \mathsf{Set}$:
  \begin{equation*}
  ...
  \begin{equation*}
    X \to B^{A}.
  \end{equation*}
  That is to say, we have a bijection between two functors $\mathsf{C}^{\text{op}} \to \mathsf{Set}$
  \begin{equation*}
...
\begin{theorem}[Yoneda lemma]
  Let $\mathcal{F}$ be a functor from $\mathsf{C}$ to $\mathsf{Set}$. Let $A \in \Obj(\mathsf{C})$. Then there is a set-isomorphism (i.e. a bijection) between the set $\Nat(h^{A}, \mathcal{F})$ of natural transformations $h^{A} \Rightarrow \mathcal{F}$ and the set $\mathcal{F}(A)$.
\end{theorem}
\begin{proof}
...
\end{proof}
TODO: add proof that the above bijection is natural in $A$.
