...
\end{equation*}
we map $\mathscr{H}_{p}$ to our proto-little-Hilbert-space $\mathscr{H}_{\bar{p}}$, act on it with some $\gamma \in \bar{\mathcal{L}}$ which fixes $\mathscr{H}_{\bar{p}}$, then map it back to where it needs to be. Since we can pull this trick for any $\alpha$ and $p$, to get an irreducible representation on $\mathscr{H}$, we only need an irreducible representation of the stabilizer of $\bar{p}$ in $\mathscr{H}_{\bar{p}}$. This turns out to be $\SL(2, \C)$.

It did not have to be this way: a priori one might have suspected that $\widetilde{\mathcal{P}}$ acts differently on each $\mathscr{H}_{p_{\mu}}$, but this turns out not to be the case: one can pick one $\bar{p}_{\mu}$ and construct from the action of the subgroup of $\widetilde{\mathcal{P}}$ which leaves $\mathscr{H}_{\bar{p}_{\mu}}$ invariant the action of $\widetilde{\mathcal{P}}$ on $\mathscr{H}_{p'_{\mu}}$ for \emph{any} other $p'_{\mu}$. 

There is a particularly easy choice: we can choose 
\begin{equation*}
  \bar{p}_{\mu} = (m,0,0,0).
\end{equation*}
With this choice, it becomes manifest that the subgroup of $\mathcal{P}$ which leaves $\bar{p}_{\mu}$ invariant is exactly the covering group of $\SO(3)$, i.e. $\SL(2, \C)$.

Thus, to obtain an irreducible representation of $\widetilde{\mathcal{P}}$ on $\mathscr{H}$, we need only find an irreducible representation of $\SL(2, \C)$ on $\mathscr{H}_{\bar{p}_{\mu}}$. These are classified by half-integers.

\subsubsection{Case 2: zero mass}
Next, let us focus on the class $0_{+}$. In this case, we do not have the `canonical' choice of $\bar{p}_{\mu} = (m,0,0,0)$. We make a different choice, however:
\begin{equation*}
  \bar{p}_{\mu} = (1,0,0,1).
\end{equation*}


\section{The Coleman-Mandula theorem}

\section{The Haag-{\L}opusza{\'n}ski-Sohnius theorem}
