...
\begin{equation*}
  \bar{p}_{\mu} = (m,0,0,0).
\end{equation*}
With this choice, it becomes manifest that the subgroup of $\mathcal{P}$ which leaves $\bar{p}_{\mu}$ invariant is exactly the covering group of $\SO(3)$, i.e. $\mathrm{Spin}(2) = \SU(2)$.
...
\begin{equation*}
  ...
  \end{pmatrix}
\end{equation*}
via the correspondence between $2 \times x$ matrices $\hat{V}$ and vectors $V^{\mu}$ in $\R^{4}$
\begin{equation*}
  V^{\mu} = \frac{1}{2} \mathrm{tr}(\hat{V} \sigma_{\mu}).
\end{equation*}

The symmetry group is generated by the following subgroups of $\SL(2, \C)$:
\begin{equation*}
  \gamma_{\varphi} = 
  \begin{pmatrix}
    e^{i \varphi} & 0 \\
...
\section{The Coleman-Mandula theorem and the The Haag-{\L}opusza{\'n}ski-Sohnius theorem}
