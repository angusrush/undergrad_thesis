...
\begin{note}
  ...
  One can also define a contravariant functor $\mathsf{C} \rightsquigarrow \mathsf{D}$ as a covariant functor $\mathsf{C}^{\text{op}} \rightsquigarrow \mathsf{D}$.
\end{note}
...
\begin{definition}[curried hom functor]
  ...
  \begin{itemize}
    ...
    \item a functor $Y_{A}\colon \mathsf{C}^{\text{op}} \rightsquigarrow \mathsf{Set}$ which maps
      \begin{itemize}
...
\begin{example}
  ...
  In fact this bijection is natural in $X$. That is to say, there is a natural bijection between the following functors $\mathsf{C}^{\text{op}} \to \mathsf{Set}$:
  \begin{equation*}
  ...
  \begin{equation*}
    \Phi_{X}\colon \Hom(X, A \times B) \to \Hom(X \times A, X \times B);\qquad f \mapsto (\pi_{A} \circ f, \pi_{B} \circ f)
  \end{equation*}
  ...
  \begin{equation*}
    X \to B^{A},
  \end{equation*}
  and vice versa.  That is to say, we have a bijection between two functors $\mathsf{C}^{\text{op}} \rightsquigarrow \mathsf{Set}$
  \begin{equation*}
...
\begin{definition}[Yoneda embedding]
  ...
  \begin{equation*}
    \mathcal{Y}\colon \mathsf{C}^{\text{op}} \rightsquigarrow [\mathsf{C}, \mathsf{Set}], \qquad A \mapsto Y^{A}.
  \end{equation*}
  where the notation $[\mathsf{C}, \mathsf{Set}]$ was defined in \hyperref[def:functorcategory]{Definition \ref*{def:functorcategory}}.

  Of course, we also need to say how $Y^{(-)}$ behaves on morphisms. Let $f\colon A \to B$.
\end{definition}
\begin{note}
  Many sources define the Yoneda embedding instead to be
  \begin{equation*}
    \mathsf{C} \rightsquigarrow [\mathsf{C}^{\text{op}}, \mathsf{Set}];\qquad A \mapsto Y_{A}.
  \end{equation*}
\end{note}

\begin{theorem}[Yoneda lemma]
  Let $\mathcal{F}$ be a (covariant) functor from $\mathsf{C}$ to $\mathsf{Set}$. Let $A \in \Obj(\mathsf{C})$. Then there is a set-isomorphism (i.e. a bijection) $\eta$ between the set $\Nat(Y^{A}, \mathcal{F})$ of natural transformations $Y^{A} \Rightarrow \mathcal{F}$ and the set $\mathcal{F}(A)$. Furthermore, this isomorphism is natural in $A$.
\end{theorem}
\begin{proof}
  A natural transformation $\Phi\colon Y^{A} \Rightarrow \mathcal{F}$ consists of a collection of $\mathsf{Set}$-morphisms (that is to say, functions) $\Phi_{B}\colon \Hom_{\mathsf{C}}(A, B) \to \mathcal{F}(B)$, one for each $B \in \Obj(\mathsf{C})$. We need to show that to each element of $\mathcal{F}(A)$ there corresponds exactly one $\Phi$. We will do this by showing that any $\Phi$ is completely determined by where $\Phi_{A}$ sends the identity morphism $1_{A} \in \Hom_{\mathsf{C}}(A, A)$, so there is exactly one natural transformation for each place $\Phi$ can send $1_{A}$.
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  The natural transformation $\Phi\colon Y^{A} \Rightarrow \mathcal{F}$ has a component $\Phi_{A}\colon \Hom_{\mathsf{C}}(A, A) \to \mathcal{F}(A)$, and $\Phi_{A}$ has to send the identity transformation $1_{A}$ \emph{somewhere}. It can send it to any element of $\mathcal{F}(A)$; let's call $\Phi_{A}(1_{A}) = a$.

  Now the naturality conditions force our hand. For any $B \in \Obj(\mathsf{C})$ and any $f\colon A \to B$, the naturality square above means that $\Phi_{B}(f)$ \emph{has to} be equal to $(\mathcal{F}f)(a)$. We get no choice in the matter.
  ...
  Well, almost. We still have to show that the bijection we constructed above is natural. To that end, let $g\colon A \to A'$. We need to show that the following square commutes.
  \begin{equation*}
    \begin{tikzcd}
      \Nat(Y^{A}, \mathcal{F})
      \arrow[r, "\eta_{A}"]
      \arrow[d, swap, "{\Nat(Y^{g}, \mathcal{F})}"]
      & \mathcal{F}(A)
      \arrow[d, "\mathcal{F}(g)"]
      \\
      \Nat(Y^{A'}, \mathcal{F})
  ...
  This deserves a lot of explanation. I haven't defined
\end{proof}
\begin{lemma}
...
\begin{proof}
  ...
  \begin{equation*}
    \mathcal{Y}_{A, B}\colon \Hom_{\mathsf{C}}(B, A) \to \Hom_{[\mathsf{C}, \mathsf{Set}]}(Y^{A}, Y^{B})
  \end{equation*}
  is a bijection.

  Fix $B \in \Obj(\mathsf{C})$, and consider the functor $Y^{B}$. By the Yoneda lemma, there is a bijection between $\Nat(Y^{A}, Y^{B})$ and $Y^{B}(A)$. By definition,
  \begin{equation*}
    Y^{B}(A) = \Hom_{\mathsf{C}}(B, A).
  \end{equation*}

  But $\Nat(Y^{A}, Y^{B})$ is nothing else but $\Hom_{[\mathsf{C}, \mathsf{Set}]}(Y^{A}, Y^{B})$, so we are done.
\end{proof}

The above is sometimes referred to as the \emph{covariant} Yoneda lemma. We also have the following.
\begin{definition}[contravariant Yoneda embedding]
  \label{def:contravariantyonedaembedding}
  For any $A \in \Obj(\mathsf{C})$, the \emph{contravariant Yoneda embedding} is the functor
  \begin{equation*}
    \mathsf{C} \rightsquigarrow [\mathsf{C}^{\text{op}}, \mathsf{Set}];\qquad A \mapsto Y_{A}.
  \end{equation*}
\end{definition}
\begin{theorem}
  The contravariant Yoneda embedding is also fully faithful.
\end{theorem}
\begin{proof}
  The proof is the same as the covariant case, mutatis mutandis.
\end{proof}
...
\end{corollary}
\begin{proof}
  We have a natural bijection $Y_{B} \Rightarrow Y_{B'}$. Since the contravariant Yoneda embedding is fully faithful, it is injective on objects up to isomorphism (\hyperref[lemma:fullyfaithfulfunctorinjectiveuptoisomorphism]{Lemma \ref*{lemma:fullyfaithfulfunctorinjectiveuptoisomorphism}}). Thus, since $Y_{B}$ and $Y_{B'}$ are isomorphic, so must be $B \simeq B'$.
\end{proof}

We have now built up enough machinery to make the proof of the associativity of the categorical product trivial. We will need to accept the trivial assertion that the cartesian product of sets is associative, i.e. for $A$, $B$, and $C$ sets, $(A \times B) \times C \simeq A \times (B \times C)$.
\begin{lemma}
  In any locally small category with products, 
\end{lemma}
...
\end{definition}
\begin{notation}
  The convention at the nLab is to denote the internal hom by square braces $[A,B]$, and this is for the most part what we will do. Unfortunately, we have already used this notation for the \emph{regular} hom functor. To remedy this, we will endeavour to add a subscript if the category to which the hom functor belongs is not clear: $[-,-]_{\mathsf{C}}$ for a hom functor internal to $\mathsf{C}$, $[-,-]_{\mathsf{Set}}$ for the standard hom functor (or the hom functor internal to $\mathsf{Set}$, which amounts to the same). 

  One often sees the internal hom functor denoted by a lower-case $\hom\colon \hom_{\mathsf{C}}(A, B)$. Many sources (for example DMOS \cite{DMOS}) distinguish the internal hom with an underline: $\underline{\Hom}_{\mathsf{C}}(A, B)$.
\end{notation}
...
\end{lemma}
\begin{proof}
  Let $A \in \Obj(\mathsf{C})$. We have the following string of natural bijections.
  \begin{align*}
...
\begin{theorem}
  Every rigid monoidal category is a closed monoidal category with internal hom object 
  \begin{equation*}
...
\end{lemma}
\begin{proof}
  We first show that if $\ker(f) = 0$, then $f$ is mono.
\end{proof}
