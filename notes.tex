...
\begin{example}
  In many locally small categories whose objects can be thought of as ``sets with extra structure'' in a natural way, we can pile structure on top of the hom sets until they can be viewed as bona fide categorical objects. It often happens that these structured hom sets coincide (up to isomorphism) with the internal hom objects.

  Take, for example $\mathsf{Vect}_{k}$. For any vector spaces $V$ and $W$, we can view $\Hom_{\mathsf{Vect}_{k}}(V, W)$ into a vector space by defining addition and scalar multiplication pointwise; we can then view $\Hom_{\mathsf{Vect}_{k}}(V, W)$ as an element of $\Obj(\mathsf{Vect}_{k})$. It turns out that this is precisely (up to ismorphism) the internal hom object $[V, W]_{\mathsf{Vect}_{k}}$.
...
\begin{proof}
  ...
  That the top left square commutes is trivial, and the right and bottom squares commute by definition. Thus, the diagram commutes.
\end{proof}
