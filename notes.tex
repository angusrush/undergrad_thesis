...
\begin{example}
  The category $\mathsf{Ab}$ of abelian groups has biproducts. 
\end{example} 
...
\begin{proof}
  ...
  The universal property for kernels tells us that there is a unique map $Z \to \ker(f)$ making the above diagram commute. But that means that $g_{1}$ and $g_{2}$ must be the same map, i.e. $g_{1} = g_{2}$.
\end{proof}
...
Here is the situation in which the Yoneda lemma is most commonly used.
\begin{corollary}
...
\begin{definition}[zero morphism]
  ...
  Let $\mathsf{C}$ be a category with zero object $0$. For any two objects $A$, $B \in \Obj(\mathsf{C})$, the \defn{zero morphism} $0_{A,B}$ is the unique morphism
  \begin{equation*}
    \begin{tikzcd}
      A
      \arrow[r]
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  We have seen (\hyperref[lemma:canonicalinjectionismono]{Lemma \ref*{lemma:canonicalinjectionismono}}) that $\iota$ is mono, and (\hyperref[lemma:canonicalsurjectionisepi]{Lemma \ref*{lemma:canonicalsurjectionisepi}}) is epi. 
  
  Now we abuse terminology by calling $\iota = \ker(f)$ and $\pi = \coker(f)$. Then we have the required decomposition.
\end{proof} 

\begin{note}
  The abuse of notation above is ubiquitous.
\end{note}

\subsection{Abelian categories}
\begin{definition}[abelian category]
  \label{def:abeliancategory}
  A pre-abelian category $\mathsf{C}$ is \defn{abelian} if for each morphism $f$, the canonical morphism
  \begin{equation*}
...
\end{lemma}
\begin{proof}
  For any morphism $f$, bracketing the decomposition $f = i \circ \bar{f} \circ p$ either as
  \begin{equation*}
    (i \circ \bar{f}) \circ p\qquad\text{or}\qquad i \circ (\bar{f} \circ p).
  \end{equation*}
...
\begin{theorem}
  All abelian categories are binormal (\hyperref[def:binormalcategory]{Definition \ref*{def:binormalcategory}}).
\end{theorem}
\begin{proof}
  We have seen that 
\end{proof}
