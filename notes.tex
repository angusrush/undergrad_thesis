...
\section{Schemes}
One way of looking at schemes is as a vast generalization of smooth manifolds. 
...
Until now, we have defined supercommutativity haphazardly, imposing the Koszul sign rule by hand as we go. In the first part of this chapter, we will see that the appearance of the Koszul sign rule is much more natural: it is really just a different sort of commutativity.

In the second section, we will apply the machinery of algebraic geometry to our new supercommutative spaces. We will arrive at a natural definition of a supermanifold.
...
\begin{example}
  ...
  A monoid internal to $\mathsf{Vect}_{k}$ is a unitary, associative $k$-algebra. Let us see that this agrees with \hyperref[def:algebraoverafield]{Definition \ref*{def:algebraoverafield}}.
...
\begin{theorem}
  A supercommutative superalgebra (\hyperref[def:supercommutativesuperalgebra]{Definition \ref*{def:supercommutativesuperalgebra}}) is a monoid in the category $\mathsf{SVect}_{k}$.
\end{theorem}
\begin{proof}
  Let $V = V_{0} \oplus V_{1}$ be a super vector space. We define, as in \hyperref[eg:algebrasaremonoidsinternaltovect]{Example \ref*{eg:algebrasaremonoidsinternaltovect}}, a bilinear map $V \times V \to V$ by pre-composing with the tensor product on $\mathsf{SVect}_{k}$:
  \begin{equation*}
  ...
  \begin{align*}
    ...
    &= \mu(\gamma_{V, V}(v_{1} \otimes v_{2})) \qquad\text{(by the commutativity diagram)}\\ 
...
Let $M$ be a $C^{\infty}$ manifold (that is, recalling our definition, an object in $\mathsf{Man}^{\infty}$).
