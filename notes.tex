...
\end{equation*}
where the even part is the direct product guaranteed by the Coleman-Mandula theorem, and the odd part is the super- component of the Poincar{\'e} algebra.
...
If one is to use Haag-{\L}opusza{\'n}ski-Sohnius theorem to motivate supersymmetry, one must navigate a minefield of caveats and grains of salt. Some of these are esoteric and uninteresting, but others are more fundamental. In order to use either theorem at all, one must make the following assumptions.
\begin{enumerate}
  \item Spacetime is 4-dimensional, and its symmetry group is the Poincar{\'e} group.

  \item One must deal directly with the symmetries of the $S$-matrix; the Haag-{\L}opusza{\'n}ski-Sohnius theorem says nothing about fundamental symmetries which are spontaneously broken.

  \item One only considers symmetries which arise via connected Lie supergroups; the Haag-{\L}opusza{\'n}ski-Sohnius does not consider the case of symmetries which are discrete, or those with multiple disconnected components.
\end{enumerate}
Further, one must hold several beliefs in order to view the Haag-{\L}opusza{\'n}ski-Sohnius theorem as motivation for supersymmetry.
\begin{enumerate}
  \item Spacetime and internal symmetries should be unified.
\end{enumerate}

\section{Questions}
\begin{enumerate}
  \item Timeline for the next few weeks? Getting worried about time constraints---Deligne's theorem is extremely technical. 

  \item Possible to get Tensor Categories, by EGNO, for the library?

  \item Do you know people (other than Urs) who are working on this sort of thing?

  \item Intertwiners as interaction vertices?
\end{enumerate}
